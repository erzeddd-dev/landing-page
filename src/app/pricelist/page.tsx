...
export const metadata: Metadata = {
  title: "Price List",
  description:
    "Daftar harga lengkap OlahData: paket olah data, proposal skripsi/tesis/disertasi, SPSS, EViews, STATA, SmartPLS, parafrase, website, dan e-modul.",
  openGraph: {
    title: "Price List · OlahData",
    description:
      "Harga transparan: olah data dari 250rb · proposal skripsi 750rb · tesis 1,5jt · disertasi 3,5jt.",
...
const proposalPackages = [
  {
    ...
    price: "750rb",
    desc: "S1 · Bab 1–3 siap diajukan.",
    features: [
      "Bab 1–3 sesuai pedoman kampus",
      "Latar belakang, rumusan, tujuan, metode",
  ...
  },
  {
    ...
    name: "Proposal Tesis",
    price: "1.500.000",
    desc: "S2 · kajian lebih dalam.",
    features: [
      "Bab 1–3 + gap research",
      "Kajian teori & desain metode S2",
      "Estimasi 7–10 hari kerja",
      "Revisi 3× sesuai brief",
    ],
    featured: true,
    badge: "Paling dipilih",
  },
  {
    id: "proposal-disertasi",
    name: "Proposal Disertasi",
    price: "3.500.000",
    desc: "S3 · novelty & state of the art.",
    features: [
      "Proposal S3 sesuai brief pembimbing",
      "Kajian literatur lebih ketat",
      "Estimasi 10–14 hari kerja",
      "Revisi 3× + konsultasi kerangka",
    ],
    featured: false,
  },
];
  ...
  return (
    <main className="min-h-screen overflow-x-hidden flex flex-col pt-24">
      <div className="max-w-5xl mx-auto px-6 pb-16 flex-grow w-full">
        ...
        <section id="proposal" className="mb-14 scroll-mt-28">
          <h2 className="font-heading text-xl font-semibold text-white/90 mb-1">
            Proposal Penelitian
          </h2>
          <p className="text-sm text-text-muted mb-6">
            Penyusunan proposal Bab 1–3. Harga mulai dari, menyesuaikan jurusan & brief.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {proposalPackages.map((pkg) => (
              <article
                key={pkg.id}
                id={pkg.id}
                className={`relative flex flex-col rounded-2xl p-5 md:p-6 scroll-mt-28 ${
                  pkg.featured
                    ? "border border-brand-secondary/40 bg-gradient-to-b from-brand-secondary/15 via-brand-primary/10 to-white/[0.02] shadow-[0_0_30px_rgba(255,0,127,0.1)]"
                    : "ui-card"
                }`}
              >
                ...
                <a
                  href={waLink(`Halo, saya tertarik ${pkg.name}. Boleh konsultasi detail & estimasi?`)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`block w-full text-center py-2.5 rounded-xl text-sm font-semibold transition-all active:scale-[0.98] ${
                    pkg.featured
                ...
                >
                  Pilih {pkg.name.replace("Proposal ", "")}
                </a>
              </article>
            ))}
          </div>
